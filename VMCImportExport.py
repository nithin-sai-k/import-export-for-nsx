...
class VMCImportExport:
    ...
    def ConfigLoader(self):
        ...
        #Tags
        self.tags_filename          = self.loadConfigFilename(config, "importConfig", "tags_filename")
    ...
    def exportSDDCMGWRule(self):
        ...
        if self.auth_mode =="token":
            ...
            myURL = (self.srcNSXmgrURL + "/policy/api/v1/infra/domains/mgw/gateway-policies/default/rules")
            response = self.invokeNSXTGET(myURL)
    ...
    def exportSDDCMGWGroups(self):
        ...
        if self.auth_mode == "token":
            ...
            myURL = (self.srcNSXmgrURL + "/policy/api/v1/infra/domains/mgw/groups")
            response = self.invokeNSXTGET(myURL)
    ...
    def export_fqdn_attribute(self):
        ...
        if response is None or response.status_code != 200:
            ...
            try:
                ...
                print(f'Could not export FQDN attribues: {e}')
                return False
    ...
    def exportSDDCTags(self):
        """Exports all security tags found in the NSX-T manager"""

        if self.auth_mode =="token":
            myURL = (self.proxy_url + "/policy/api/v1/infra/tags")
            response = self.invokeVMCGET(myURL)
        else:
            myURL = (self.srcNSXmgrURL + "/policy/api/v1/infra/tags")
            response = self.invokeNSXTGET(myURL)
           
        if response is None or response.status_code != 200:
            return False
        json_response = response.json()
        sddc_tags = json_response['results']

        fname = self.export_path / self.tags_filename
        with open(fname, 'w') as outfile:
    ...
    def exportSDDCVMs(self):
        ...
        if self.auth_mode =="token":
            myURL = (self.proxy_url + "/api/v1/fabric/virtual-machines")
            response = self.invokeVMCGET(myURL)
        else:
            myURL = (self.srcNSXmgrURL + "/api/v1/fabric/virtual-machines")
            response = self.invokeNSXTGET(myURL)
           
        if response is None or response.status_code != 200:
            return False
        json_response = response.json()
        sddc_vms = json_response['results']

        fname = self.export_path / self.vms_filename
        with open(fname, 'w') as outfile:
            json.dump(sddc_vms, outfile,indent=4)
        return True 


    def exportSDDCVMVIFs(self):
        """Exports all VM VIFs found in the NSX-T manager"""

        if self.auth_mode =="token":
            myURL = (self.proxy_url + "/api/v1/fabric/vifs")
            response = self.invokeVMCGET(myURL)
        else:
            myURL = (self.srcNSXmgrURL + "/api/v1/fabric/vifs")
            response = self.invokeNSXTGET(myURL)
        ...
        json_response = response.json()
        vm_vifs = json_response['results']

        fname = self.export_path / self.vm_vifs_filename
        with open(fname, 'w') as outfile:
    ...
    def exportSDDCListPublicIP(self):
        ...
        if self.auth_mode =="token":
            ...
            myURL = (self.srcNSXmgrURL + "/cloud-service/api/v1/infra/public-ips")
            response = self.invokeNSXTGET(myURL)
